# Ciara Frances P. Comendador - Portfolio

## About Me
Information Systems student and Business Documentation Specialist with experience in process mapping, BPMN, and project management. Passionate about improving business processes and building efficient systems.

## Portfolio UI
https://www.figma.com/proto/N6TFDHwueaaWvIL9JBLajC/My-Portfolio-UI

## CV
[Comendador - Resume.pdf](https://github.com/USERR4N/ciara-comendador-portfolio/blob/main/Comendador%20-%20Resume.pdf)

## Projects

### 1. ArchiLink
- Description: Platform connecting clients with architects and engineers, inspired by LinkedIn and service-based systems.
- Tech Stack: TypeScript, React (assumed), JSON
- GitHub: https://github.com/USERR4N/Archilink-Mobile-App.git

### 2. Pawlax
- Description: Web-based project (pet-related system) developed using full-stack technologies.
- Tech Stack: HTML, CSS, JavaScript
- GitHub: https://github.com/USERR4N/PawLax-Pet-Service-Grooming-Website.git

### 3. Metabase Pharmacy Inventory
- Description: Pharmacy inventory system with dashboards and analytics using Metabase for visualization.
- Tech Stack: Metabase, DBeaver, SQL
- GitHub: https://github.com/USERR4N/Metabase-Pharmacy-Inventory.git

### 4. Aegis – Hackathon Jam 2026
- Description: Cybersecurity-focused solution inspired by a browser extension, designed to enhance user privacy and protection.
- Tech Stack: TypeScript, Web Technologies
- GitHub: https://github.com/USERR4N/Hackathon-Jam-2026-Aegis.git
- Website: https://aegis-nine-xi.vercel.app/

### 5. Students Dropout and Success Dataset (Jupyter Notebook)
- Description: Data analysis project exploring student dropout and success patterns using statistical and visualization techniques.
- Tech Stack: Python, Jupyter Notebook, Data Analysis
- GitHub: https://github.com/USERR4N/Jupyter-Students-Dropout-and-Success-Dataset.git

### 6. Students Scores Dataset (Google Colab)
- Description: Data analysis project examining student performance and identifying key trends using Google Colab.
- Tech Stack: Python, Google Colab
- GitHub: https://github.com/USERR4N/Google-Colab-Student-Scores-Dataset.git

### 7. Plug&Play - Retail Store Website
- Description: Full-stack retail web application, featuring product listings, user interaction, and backend functionality. Designed to simulate a real-world e-commerce experience.
- Tech Stack: HTML, Javascript, CSS
- GitHub: https://github.com/USERR4N/Plug-Play.git
- Website: https://plug-and-play-black.vercel.app/

### 8. BDO Historical Data – Kaggle Class Competition
- Description: Data analysis project using historical financial data for insights and predictions במסגרת a class competition.
- Tech Stack: Python, Kaggle, Data Analysis
- GitHub: https://github.com/USERR4N/Kaggle-Competition-BDO-Historical-Data.git

### 9. Ciel d'Or – Hackathon 2024
- Description: Designed a UI/UX prototype for a French-Filipino restaurant booking platform, focusing on seamless reservations and elegant user experience.
- Tech Stack: Figma (UI/UX Design, Prototyping)
- Figma Prototype: https://www.figma.com/proto/KCqvNMoVLsVIGrBbfuAS2x/Ciel-d-Or-Website?node-id=0-1&t=N4iN9ex7B1cTgl34-1

### 10. CIIT Fluxx – Shark Tank Convention 2025
- Description: Designed a Netflix-inspired UI/UX platform for showcasing student portfolios, allowing students, professors, and professionals to explore creative work in a structured and engaging interface.
- Tech Stack: Figma (UI/UX Design, Prototyping)
- Figma Prototype: https://www.figma.com/proto/C1p6E8flI5lILE6Ci5DC3k/Finals-Fluxx?node-id=0-1&t=2lW8KQTxd4axctf0-1

### 11. Lila Tinig
- Description: Designed a UI/UX prototype for an advocacy-driven storytelling platform, enabling users to share and amplify voices through digital narratives.
- Tech Stack: Figma (UI/UX Design, Prototyping)
- Figma Prototype: https://www.figma.com/proto/dR1iLCNEcZhGePMDSSAcoJ/Lila-Tinig?node-id=0-1&t=BPEzonfk6uixcCQd-1

### 12. YelloX Business Process Mapping
- Description: Conducted process analysis and created BPMN workflows (Order-to-Cash, Warehousing, Quote-to-Cash) for real businesses.
- Tech Stack: BPMN, Lucidchart, Process Analysis

### 13. Enjoy Your Coffee (EYC Café Case Study)
- Description: Analyzed business operations, identified pain points, and proposed process improvements through system design and documentation.
- Tech Stack: Business Analysis, Financial Analysis, Documentation

## Certifications & Achievements

- **Best Overall Project – 2nd Place**  
  CIIT Hackathon Jam 2026  
  Developed a cybersecurity-focused solution under the theme "Mask"

- **1st Place – Sales Blitz (Food Category)**  
  CIIT Emerging Startups Convention (CESCON)  
  Led end-to-end business execution and sales strategy

## Skills
- BPMN & Process Mapping
- Business Analysis
- Project Management
- Power BI
- Microsoft Excel
